...
import { Loader2, CheckCircle2, AlertCircle, FileSpreadsheet, Download } from "lucide-react";
...
export interface CsvExpectedField {
    key: string;
    label: string;
    required?: boolean;
...
export interface CsvUploadDialogProps {
    ...
    title: string;
    description: string;
    expectedFields: CsvExpectedField[];
    onImport: (data: any[]) => Promise<CsvImportResult>;
    sampleData?: any[];
        ...
        if (errorObj.error) {
            try {
                ...
                if (Array.isArray(zodIssues)) {
                    return zodIssues.map((issue: any) => {
...
export function CsvUploadDialog({
    ...
    const handleDownloadTemplate = () => {
        ...
        toast({ title: "Template Downloaded", description: `Saved as ${templateFilename || "template.csv"}` });
    };
    ...
    const previewFields = expectedFields.filter(
        (f) => !f.hideInPreview && !!mapping[f.key] && mapping[f.key] !== "none" && headers.includes(mapping[f.key])
    );
                ...
                <div className="flex-1 overflow-y-auto px-6 py-4 custom-scrollbar">
                    ...
                    {step === "upload" && (
                        <div className="space-y-4">
                            <BaseFileUpload
                                ...
                                icon={<FileSpreadsheet className="w-7 h-7 text-[#C7AE6A]" />}
                                title="Click or drag CSV file to upload"
                                subtitle="CSV · Max 2 MB"
                            ...
                            {sampleData && sampleData.length > 0 && (
                                <div className="flex justify-center">
                                    <Button
                                        ...
                                        className="gap-2 bg-black/40 border-[#222] text-xs h-9 hover:text-[#C7AE6A] hover:bg-black/60 transition-all font-bold"
                                    >
                    ...
                    {step === "map" && (
                        <div className="space-y-6">
                            {/* File info bar */}
                            {file && (
                                <div className="flex items-center gap-3 px-4 py-3 bg-black/40 rounded-xl border border-[#222]">
                                    <FileSpreadsheet className="w-4 h-4 text-[#C7AE6A]" />
                                    ...
                                    <span className="text-[10px] text-gray-500">{csvData.length} rows</span>
                                </div>
                            )}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-4 text-sm">
                                    ...
                                    {expectedFields.map((field) => (
                                        <div
                                            key={field.key}
                                            className="flex items-center justify-between h-10 px-3 bg-black/40 rounded-lg border border-[#222]"
                                        >
                        ...
                        <div className="space-y-4">
                            <div className="bg-[#C7AE6A]/10 border border-[#C7AE6A]/20 p-4 rounded-xl flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-[#C7AE6A]/70">
                                        {csvData.length} rows ready for batch processing.
                                    </p>
                                </div>
                            </div>
                            <div className="border border-[#222] rounded-xl overflow-hidden max-h-[400px] overflow-y-auto bg-black/20 custom-scrollbar">
                                <Table>
                                    <TableHeader className="bg-[#111] sticky top-0 z-10 shadow-[0_1px_0_rgba(255,255,255,0.05)]">
                                        <TableRow className="border-[#222] hover:bg-transparent">
                                            {previewFields.map((f) => (
                                                <TableHead
                                                    key={f.key}
                                                    className="text-[10px] font-black uppercase tracking-widest py-3 text-[#C7AE6A]"
                                                >
                                                    {f.label}
                                                </TableHead>
                                            ))}
                                    ...
                                    <TableBody>
                                        {csvData.slice(0, MAX_PREVIEW_ROWS).map((row, i) => (
                            ...
                            <div className="flex justify-between items-center px-1">
                                <p className="text-[10px] text-gray-500 italic">
                                    Showing {Math.min(csvData.length, MAX_PREVIEW_ROWS)} of {csvData.length} rows
                                </p>
                        ...
                        <div className="flex flex-col items-center justify-center h-64 gap-4">
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-xs text-muted-foreground mt-1">
                                    Writing documents in batches of 500. This may take a moment.
                                </p>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="flex flex-col gap-6">
                            ...
                            <div className="grid grid-cols-3 gap-4 w-full">
                                ...
                                <div className="p-4 bg-[#C7AE6A]/5 rounded-2xl border border-[#C7AE6A]/20">
                                    <p className="text-[10px] font-black uppercase text-[#C7AE6A]/60 tracking-widest mb-1">
                                        Success
                                    </p>
                                    <p className="text-2xl font-black text-[#C7AE6A]">{importResults.success}</p>
                                </div>
                                <div
                                    className={`p-4 rounded-2xl border ${importResults.failed > 0
                                        ? "bg-red-500/5 border-red-500/20"
                                        : "bg-black/40 border-[#222]"
                                        }`}
                                >
                                    <p
                                        className={`text-[10px] font-black uppercase tracking-widest mb-1 ${importResults.failed > 0 ? "text-red-500/60" : "text-gray-500"
                                            }`}
                                    >
                                        Failed
                                    </p>
                                    <p
                                        className={`text-2xl font-black ${importResults.failed > 0 ? "text-red-500" : "text-white"
                                            }`}
                                    >
                            ...
                            {importResults.failures.length > 0 && (
                                <div className="space-y-3">
                                    ...
                                    <div className="border border-red-500/20 rounded-xl overflow-hidden max-h-48 overflow-y-auto bg-red-500/5 custom-scrollbar">
                                        <Table>
                                            <TableBody>
                                                {importResults.failures.map((f, i) => (
                                                    <TableRow
                                                        key={i}
                                                        className="border-red-500/10 hover:bg-red-500/10"
                                                    >
                                                        <TableCell className="py-2 text-xs font-bold text-red-200">
                                                            {f.name}
                                                        </TableCell>
                                                        <TableCell className="py-2 text-[10px] text-red-400 font-mono">
                                                            <div className="flex flex-col gap-1">
                                                                {(formatErrorMessage(f.error) as string[]).map(
                                    ...
                                    <p className="text-[11px] text-gray-500 text-center italic">
                                        Please correct the issues above in your CSV file and re-upload the failed
                                        records.
                                    </p>
                                </div>
                            )}
