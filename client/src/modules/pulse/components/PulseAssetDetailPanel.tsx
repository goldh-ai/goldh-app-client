...
interface PulseAssetDetailPanelProps {
    symbol: string;
    onClose: () => void;
}
...
export default function PulseAssetDetailPanel({ symbol, onClose }: PulseAssetDetailPanelProps) {
    ...
    return (
        <div
            className="fixed inset-y-0 right-0 w-full md:w-[520px] bg-[#0a0a0a] border-l border-[#222] z-50 flex flex-col shadow-2xl animate-in slide-in-from-right duration-300"
            data-testid="pulse-asset-detail-panel"
            role="dialog"
            aria-label={asset ? `${asset.name} details` : 'Asset details'}
        >
            {/* ─── Header bar ─────────────────────────────────────────────── */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#222] bg-[#0a0a0a] shrink-0">
                <div className="flex items-center gap-3">
                    {asset ? (
                        <>
                            <span className="text-xl font-bold text-white">{asset.symbol}</span>
                            <span className="text-xs text-[#6b6b6b] font-bold uppercase">{asset.name}</span>
                        </>
                    ) : (
            ...
            {/* ─── Footer bar ─────────────────────────────────────────────── */}
            <div className="px-6 py-4 border-t border-[#222] bg-[#0a0a0a] shrink-0 flex items-center justify-between">
                <button
                    onClick={onClose}
